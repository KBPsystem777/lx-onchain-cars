...
export function RentalChecklist({
  ...
  const handleItemChange = (id: string, checked: boolean) => {
    const newItems = items.map((item) => (item.id === id ? { ...item, checked } : item))
    setItems(newItems)
    onSave?.(newItems)
  }
                ...
                <div className="space-y-1 leading-none">
                  <label
                    ...
                    className={`text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 ${
                      item.required ? "after:content-['*'] after:ml-0.5 after:text-red-500" : ""
                    }`}
                  >
                    {item.label}
                  </label>
          ...
          <CardFooter className="flex flex-col space-y-4">
            {!allRequiredChecked && (
              <div className="flex items-start space-x-2 text-sm text-amber-600 bg-amber-50 p-3 rounded-md w-full">
                ...
                <p>Please complete all required items marked with an asterisk (*) before proceeding.</p>
              </div>
            )}
            <Button
              className="w-full"
              ...
              onClick={onComplete}
              variant={allItemsChecked ? "default" : allRequiredChecked ? "outline" : "secondary"}
            >
              {allItemsChecked ? (
                <>
